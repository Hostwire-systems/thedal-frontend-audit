...
interface AddressedVotersBreakdownModalProps {
  visible: boolean;
  onClose: () => void;
  electionId: number;
  onExport?: () => void;
...
const AddressedVotersBreakdownModal: React.FC<AddressedVotersBreakdownModalProps> = ({
  visible,
  onClose,
  electionId,
  onExport
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Error fetching addressed voters breakdown:", error);
      message.error("Failed to load addressed voters breakdown");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal
      title="Addressed Voters Breakdown by Part"
      open={visible}
      onCancel={onClose}
      width={900}
      footer={[
...
export default AddressedVotersBreakdownModal;
