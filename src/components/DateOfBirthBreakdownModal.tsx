...
interface DateOfBirthBreakdownModalProps {
  visible: boolean;
  onClose: () => void;
  electionId: number;
}
...
const DateOfBirthBreakdownModal: React.FC<DateOfBirthBreakdownModalProps> = ({
  visible,
  onClose,
  electionId,
}) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const tableData: PartData[] = statsArray.map((stat) => {
        ...
        const maleDobCount = stat.maleDateOfBirthCount || 0;
        const maleVoterCount = stat.male || 0;
        const malePercentage = maleVoterCount > 0 
          ? ((maleDobCount / maleVoterCount) * 100).toFixed(2) 
          : "0.00";

        const femaleDobCount = stat.femaleDateOfBirthCount || 0;
        const femaleVoterCount = stat.female || 0;
        const femalePercentage = femaleVoterCount > 0 
          ? ((femaleDobCount / femaleVoterCount) * 100).toFixed(2) 
          : "0.00";

        const othersDobCount = stat.transgenderDateOfBirthCount || 0;
        const othersVoterCount = stat.transgender || 0;
        const othersPercentage = othersVoterCount > 0 
          ? ((othersDobCount / othersVoterCount) * 100).toFixed(2) 
          : "0.00";

        const totalDobCount = stat.dateOfBirth || 0;
        const totalVoterCount = stat.totalVoters || 0;
        const totalPercentage = totalVoterCount > 0 
          ? ((totalDobCount / totalVoterCount) * 100).toFixed(2) 
          : "0.00";
      ...
      const partTotals = tableData.reduce(
        (acc, row) => {
          acc.maleCount += row.maleDobCount;
          acc.maleVoters += row.maleVoterCount;
          acc.femaleCount += row.femaleDobCount;
          acc.femaleVoters += row.femaleVoterCount;
          acc.othersCount += row.othersDobCount;
          acc.othersVoters += row.othersVoterCount;
          acc.totalCount += row.totalDobCount;
      ...
      const unmappedMaleCount = Math.max(0, (overallStats.maleDateOfBirthCount || 0) - partTotals.maleCount);
      const unmappedMaleVoters = Math.max(0, (overallStats.male || 0) - partTotals.maleVoters);
      const unmappedFemaleCount = Math.max(0, (overallStats.femaleDateOfBirthCount || 0) - partTotals.femaleCount);
      const unmappedFemaleVoters = Math.max(0, (overallStats.female || 0) - partTotals.femaleVoters);
      const unmappedOthersCount = Math.max(0, (overallStats.transgenderDateOfBirthCount || 0) - partTotals.othersCount);
      const unmappedOthersVoters = Math.max(0, (overallStats.transgender || 0) - partTotals.othersVoters);
      const unmappedTotalCount = Math.max(0, (overallStats.dateOfBirth || 0) - partTotals.totalCount);
      ...
      if (
        ...
        tableData.push({
          ...
          maleDobCount: unmappedMaleCount,
          ...
          femaleDobCount: unmappedFemaleCount,
          ...
          othersDobCount: unmappedOthersCount,
          ...
          totalDobCount: unmappedTotalCount,
      ...
      console.error("Error fetching date of birth breakdown:", error);
      message.error("Failed to load date of birth breakdown");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal
      title="Date of Birth Breakdown by Part"
      open={visible}
      onCancel={onClose}
      width={1400}
      footer={null}
...
export default DateOfBirthBreakdownModal;
